...
static gboolean push_data (CustomData *data) {
  ...
  GstMapInfo mapinfo;
  GstMapFlags mapflags = GST_MAP_READWRITE;
  gst_buffer_map(buffer, &mapinfo, mapflags);
  raw = (gint8 *) (mapinfo.data);
...
int main(int argc, char *argv[]) {
  ...
  g_print ("audio_caps_text( %s ).\n", audio_caps_text);
  audio_caps = gst_caps_from_string (audio_caps_text);
#else
  audio_caps = gst_caps_from_string (SAMPLE_CAPS);
#endif
  ...
  g_signal_connect (data.app_sink, "new-sample", G_CALLBACK (new_buffer), &data);
